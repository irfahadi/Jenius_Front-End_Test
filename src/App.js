...
function App() {
  ...
  useEffect(() => {
    const loadContact = async () => await dispatch(fetchContact())
    loadContact()
  }, [dispatch])
  ...
  return (
    ...
    <div className="w-full sm:w-1/4 bg-white text-gray-500 text-center text-2xl font-bold py-10 sm:min-h-screen sm:h-auto h-40 shadow-lg ">Contacts</div>
    <div className="w-full sm:w-3/4 p-4 shadow-md ">
        {loading ? (
          "Loading..."
        ) : (
        <Table Contact = {entities}
        showModal = {showModal}
        editRow ={editRow}
